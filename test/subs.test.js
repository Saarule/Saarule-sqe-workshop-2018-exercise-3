...
describe('Substitute for function with args type: number', () => {
    it('without if condition', () => {
        let ansNoEvalAndArgs = subedCode(parseCode('function foo(x){\nlet a = x + 7;\nreturn x + 7;\n}'), {}, '', false);
        let ansWithEvalAndArgs = subedCode(parseCode('function foo(x){\nlet a = x + 7;\nreturn x + 7;\n}'), {}, '1', true);
        assert.deepEqual(codeViewer(ansNoEvalAndArgs['newJson'], ansWithEvalAndArgs['greenLines'], ansWithEvalAndArgs['redLines'], ansNoEvalAndArgs['listRowsToIgnore']),
    ...
    it('with if condition', () => {
        ...
        let ansNoEvalAndArgs = subedCode(parseCode(code), {}, '', false);
        let ansWithEvalAndArgs = subedCode(parseCode(code), {}, args, true);
        assert.deepEqual(codeViewer(ansNoEvalAndArgs['newJson'], ansWithEvalAndArgs['greenLines'], ansWithEvalAndArgs['redLines'], ansNoEvalAndArgs['listRowsToIgnore']),
            [
...
describe('Substitute for empty function', () => {
    it('without if condition', () => {
        let ansNoEvalAndArgs = subedCode(parseCode('function foo(){\nvar a;\nlet a = 2;\na = a * a;\nreturn a;\n}'), {}, '', false);
        let ansWithEvalAndArgs = subedCode(parseCode('function foo(){\nlet a = 2;\nreturn a;\n}'), {}, '', true);
        assert.deepEqual(codeViewer(ansNoEvalAndArgs['newJson'], ansWithEvalAndArgs['greenLines'], ansWithEvalAndArgs['redLines'], ansNoEvalAndArgs['listRowsToIgnore']),
